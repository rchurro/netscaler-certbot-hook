...
- ✅ **Multitenant Support** - Configurable SSL directory locations for partitioned environments
...
| `SSL_LOCATION` | No | `/nsconfig/ssl` | SSL certificate directory location on NetScaler |
...
export SSL_LOCATION=/nsconfig/ssl
```
...
| `--ssl-location` | No | `/nsconfig/ssl` | SSL certificate directory location on NetScaler |
...
For NetScaler deployments with partitions (multitenant environments), certificates may be stored in partition-specific directories. Use the `SSL_LOCATION` environment variable or `--ssl-location` argument to specify the correct path:

**Environment Variable:**
```bash
export SSL_LOCATION="/nsconfig/partitions/partition_name/ssl"
netscaler-certbot-hook --name example.com
```

**Command Line Argument:**
```bash
netscaler-certbot-hook --name example.com --ssl-location "/nsconfig/partitions/partition_name/ssl"
```

**Common Partition Paths:**
- `/nsconfig/partitions/PARTITION-NAME/ssl` - Standard partition location
- `/nsconfig/partitions/partition_name/ssl` - Your partition_name partition
- `/nsconfig/partitions/PROD/ssl` - Production partition
- `/nsconfig/partitions/DEV/ssl` - Development partition

**Note:** The SSL location must exist on the NetScaler appliance and be writable by the NetScaler service.
