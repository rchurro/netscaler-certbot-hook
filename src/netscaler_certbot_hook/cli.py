...
Environment Variables:
    ...
    SSL_LOCATION: SSL certificate directory location on NetScaler (default: /nsconfig/ssl)
...
add_args = {
  ...
  '--ssl-location': {
    'metavar': '<string>',
    'help': 'SSL certificate directory location on NetScaler (default: /nsconfig/ssl)',
    'type': str,
    'default': None,
    'required': False,
  },
...
def nitro_upload(nitro_client: nitro.NitroClient, source_file: str, target_filename: str, ssl_location: str = '/nsconfig/ssl') -> Dict[str, Any]:
    """Upload a file to the NetScaler SSL directory.

    Args:
        nitro_client (NitroClient): Configured NITRO API client instance.
        source_file (str): Local path to the file to upload.
        target_filename (str): Target filename on the NetScaler.
        ssl_location (str): SSL directory location on NetScaler (default: /nsconfig/ssl).
...
def nitro_delete(nitro_client: nitro.NitroClient, filename: str, ssl_location: str = '/nsconfig/ssl') -> Dict[str, Any]:
    """Delete a file from the NetScaler SSL directory.
    ...
    """
    # URL encode the ssl_location for the parameter
    encoded_location = ssl_location.replace('/', '%2F')
    return nitro_client.request(
        'delete',
        endpoint='config',
        objecttype='systemfile',
        objectname=filename,
        params={'args': f'filelocation:{encoded_location}'},
    )


def nitro_install_cert(nitro_client: nitro.NitroClient, name: str, cert: Optional[str] = None,
                       key: Optional[str] = None, update: bool = False, no_domain_check: bool = False, 
                       ssl_location: str = '/nsconfig/ssl') -> Dict[str, Any]:
    ...
    if cert:
        data['sslcertkey']['cert'] = "{}/{}".format(ssl_location, cert)
    if key:
        data['sslcertkey']['key'] = "{}/{}".format(ssl_location, key)
    if no_domain_check:
...
def get_config(args: argparse.Namespace) -> Dict[str, Union[str, bool, int]]:
    ...
    config = {
        ...
        'ssl_location': args.ssl_location or os.getenv('SSL_LOCATION', '/nsconfig/ssl'),
    ...
    if check_chain:
        ...
        if installed_serial == chain_serial:
            ...
            if config['update_chain']:
                ...
                nitro_upload(nitro_client, config['chain_file'], chain_filename, config['ssl_location'])
                logger.info("updating chain certificate with serial %s", chain_serial)
                nitro_install_cert(nitro_client, config['chain_name'], cert=chain_filename, update=True, no_domain_check=config['no_domain_check'], ssl_location=config['ssl_location'])
            else:
        ...
        nitro_install_cert(nitro_client, config['chain_name'], cert=chain_filename, ssl_location=config['ssl_location'])
...
def install_or_update_certificate(nitro_client: nitro.NitroClient, config: Dict[str, Union[str, bool, int]],
    ...
    nitro_upload(nitro_client, config['cert_file'], cert_filename, config['ssl_location'])
    logger.info("uploading private key as %s", key_filename)
    nitro_upload(nitro_client, config['privkey_file'], key_filename, config['ssl_location'])
    ...
    nitro_install_cert(nitro_client, config['cert_name'], cert=cert_filename, key=key_filename, update=update, no_domain_check=config['no_domain_check'], ssl_location=config['ssl_location'])
